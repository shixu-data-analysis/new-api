...
assert.match(workflow, /\{\{json \.SBOM\}\}/);
assert.match(workflow, /select\(has\("SPDX"\)\).*SPDX\.SPDXID/);
assert.match(workflow, /\{\{json \.Provenance\}\}/);
assert.match(workflow, /select\(has\("SLSA"\)\).*SLSA\.buildType/);
