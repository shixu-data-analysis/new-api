...
- **类型检查**：通常每次改动 TypeScript 或 TSX 代码后执行类型检查（如 `bun run typecheck`）；Canvas 大型 UI 的例外与最终时机以 [Web 入口](../../web/AGENTS.md#canvas-ui-iteration-exception) 为准。若出现类型错误，须修复至无错误为止，不得遗留。
