# New API backend rules

Read this file in full before changing Go code, backend configuration, database access or migrations, relay/provider code, billing, or backend tests. The backend rules below are preserved from `new-api-upstream/AGENTS.md`; shared project facts and protected-attribution governance remain in that repository file.

## Backend rules

Paths and commands below are relative to the New API repository root. Backend localization uses `nicksnyder/go-i18n/v2`, with en/zh resources in `i18n/`.
